...
%
% This code integrates a RW + soliton BC coming from the right and
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
x0 = 650; ur = 3*(-1)+1/2; ul = 3*(-3/2)+1/2; B = .3;
x0sol = x0-20; A = 5/2; C = -sqrt(-2*ul + 1 - 2/3*A); %C = -sqrt(-2*ur + 1 - 2/3*A) for left-moving soliton
% note that C and q (from kp solutions) have opposite signs. C <0 =>
% left-moving soliton
% need the -2u_l for solitons starting on left!!!! This comes from the
% constraint on the relationship between a and q from KPII, converted to
...
qfun = @(x,y) abs((x)./y).*cos( 1/3*acos(thetafun(x,y)) - 0*pi/3); %analytical solution q(x/y) (for kp)
xsol = x0sol-x0;
dx = 2*L/N;
xtraj = xsol;
ttraj = 0:dt:tmax;
  ...
  for n = 1:nmax
    ...
    %soliton trajectory
      ...
      elseif xsol_old > xright-x0
          ...
          % changed all the below to positive ie dx/dy = q...That seems
          % iffy
          c1 = -qfun(xsol,t);
...
%overlay exact soliton trajectory and 
  ...
  plot(xtraj+x0,ttraj(1:length(xtraj)),'r','LineWidth',1,'LineStyle','--');
  axis([-L+200 L 0 t])
